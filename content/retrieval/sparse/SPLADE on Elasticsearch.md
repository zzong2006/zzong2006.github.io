...
다만 BM25은 term frequency(작은 정수)만 저장하므로 엔트리당 약 3-4 bytes로 더 작을 수 있고, SPLADE는 float weight를 저장해서 약 5-6 bytes로 더 클 수 있다. 아래 표에서는 동일한 5 bytes/entry로 단순 비교한 것이다.

## F.3) D_avg 별 인덱스 크기 (N=1억 문서 기준)

| 모델/설정              | D_avg   | 총 엔트리 수 | 추정 인덱스 크기    | BM25 대비 |
| ------------------ | ------- | ------- | ------------ | ------- |
| BM25 (참고)          | 약 200    | 200억    | **약 100 GB**  | 1x      |
| 강한 정규화 SPLADE      | 약 300    | 300억    | **약 150 GB**  | 1.5x    |
| `splade-ko-v1`     | 약 650    | 650억    | **약 325 GB**  | 3.3x    |
| `gte-multilingual` | 약 2,700  | 2,700억  | **약 1.35 TB** | 13.5x   |
| 약한 정규화             | 약 19,000 | 1.9조    | **약 9.5 TB**  | 비현실적    |
