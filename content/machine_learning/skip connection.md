...
$F$ 가 층이 수행하는 변환이고, $x$ 가 건너뛰어 더해지는 입력이다. [[residual connection]] 과 같은 개념이며, 층이 $y$ 전체가 아니라 $y - x$ 라는 잔차만 학습하면 된다는 관점에서 그렇게 부른다.

# B) 왜 필요한가

깊은 신경망에서 gradient 는 역전파를 거치며 층마다 미분값이 곱해진다. [[activation function]] 의 미분이 1 보다 작으면 이 곱이 층 수만큼 반복되어 앞쪽 층에 도달할 즈음엔 0 에 가까워진다. gradient vanishing 이다.
