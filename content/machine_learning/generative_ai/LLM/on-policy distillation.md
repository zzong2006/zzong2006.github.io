...
RL과 비교할 때 조건도 하나 붙는다. judge 모델이 판정문을 생성해야 하는 구조라면 RL 쪽이 확실히 비싸지만, 수학 답 대조나 단위테스트처럼 기계적으로 채점하는 RL이라면 보상 계산이 거의 공짜다. 그 경우 스텝당 비용은 RL이 오히려 싸고, on-policy distillation이 이기는 건 스텝 수 쪽이다.
...
비용은 두 축으로 나눠 봐야 한다. **스텝당 비용** 은 SFT가 가장 싸다(생성이 없다). RL과 on-policy distillation은 둘 다 학생 rollout을 새로 뽑아야 해서 비슷한데, 보상을 judge 모델로 매기면 RL이 더 비싸고 기계적으로 채점하면 RL이 더 싸다. 반면 **목표 성능에 도달하기까지의 총비용** 은 순서가 뒤집혀서 on-policy distillation이 가장 싸다. 스텝이 훨씬 적게 들어서다.
...
4. **KL을 어느 토큰에서 재는지도 손잡이다.** 학생이 실제로 뽑은 토큰에서 재는 쪽이 argmax 토큰만 쓰는 쪽(Top-1)보다 안정적이다
...
네 번째 항목은 KL을 어느 토큰에서 재느냐다. 매 위치에서 vocabulary 전체에 대해 KL을 다 계산하는 건 비싸서, 일부 토큰에서만 재는 구현이 흔하다. 어느 토큰을 고르느냐가 학습 안정성을 가른다.
...
Top-1은 매번 확률이 가장 높은 토큰만 본다. 이미 mode-seeking인 reverse KL 위에 "제일 높은 것을 더 높여라"를 얹는 꼴이다. 학생이 한 토큰에 확률을 몰수록 그 토큰은 더 확실한 argmax가 되고, 다음 스텝에서도 같은 토큰만 강화된다. 방금 말한 entropy collapse로 굴러떨어지는 양의 되먹임이다. 게다가 학생이 실제로 샘플링한 토큰이 argmax가 아니었다면 바로 그 실수에는 신호가 가지 않는다.
