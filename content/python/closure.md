...
그렇다면 도대체 클로저 함수는 어디다가 변수를 저장하는 것인가? 해답은 `__closure__` 라는 매직 메소드에 존재한다. `__closure__` 라는 메소드는 `cell` 이라는 `tuple` 타입을 반환하는데, 이 `cell` 의 `cell_contents` 안에는 `message` 변수의 값이 포함되어 있다.
