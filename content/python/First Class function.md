...
[[Python]] 은 퍼스트 클래스 함수 (First Class function) 를 지원한다.
...
# B) (1) 함수 자체를 인자 (argument) 로써 다른 함수에 전달할 수 있음
...
클로저 함수 `log_message` 가 `msg` 를 실제로 기억하고 있는지 알아보기 위해, `logger` 함수를 global namespace 에서 완전히 지운 후, `log_message` 를 호출하여 보겠다.
...
- log_hi() # logger 가 지워진 뒤에도 Log: Hi" 가 출력됩니다.
```
