...
* For a continuous random variable, the mode is defined as a peak in the density $p(\boldsymbol{x})$.
* In other words, the mode $x_{*}$ of a distribution $p(x)$ is the state of $x$ at which the distribution takes its highest value, $x_{*}=\underset{x}{\operatorname{argmax}}p(x)$.
	* 만약 $\boldsymbol{X}$ 가 이산 랜덤 변수라면, pmf 가 가장 큰 값을 가질 수 있는 $\boldsymbol{X}=x$ 가 mode 가 된다.
