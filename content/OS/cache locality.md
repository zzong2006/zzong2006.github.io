...
**캐시 지역성이 좋고 나쁘다는 것은 무슨 의미를 가지는가?**
CPU 는 일반적으로 특정 메모리에 접근할 때, 해당 메모리 근처의 chunks 들을 cache 에 미리 올려둔다. 그리고, 근처 메모리들을 접근하게 되면, 캐시를 통해 빠르게 접근할 수 있다.

Array 는 연속된 메모리 블럭을 가지고, linked list 는 각 노드의 메모리가 연속성을 보장하지 않는다.

아래는 각 자료구조의 memory layout 을 나타낸다.
