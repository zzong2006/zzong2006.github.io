...
두개이상의 테이블이나 데이터베이스를 연결하여 데이터를 검색하는 방법이다. 주로 정규화된 릴레이션을 통해 원하는 정보를 찾고자 할때 사용한다.
