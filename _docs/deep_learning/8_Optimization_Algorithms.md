...
위와같은 loss function의 contour가 있다고 가정하자. red point 까지 가기 위해서는 세로는 짧게, 가로는 길게 움직이고 싶을 것이다.
...
* 위 그림은 가로는 $w$ 이고, 세로는 $b$일 때의 loss function이라 가정하자.
...
![Adam Optimization Algorithm. An effective optimization algorithm | by  Kurtis Pykes | Towards Data Science](https://i.loli.net/2020/10/29/Tpi5D9zd3XKFsnq.png)
