...
* MC와 TD는 둘 다 경험을 통해 $v_\pi$의 추정값 $V$를 찾는다.

### Constant-$\alpha$ MC

* MC 방법(특히 every-visit)은 $V(S_t)$를 추정하기 위해 **실제** return 값 $G_t$를 알아야한다.
  ...
  * episode가 완료될 때 $G_t$를 거꾸로 찾아가므로, episode가 완료되기까지 위의 update는 불가능하다.  
...
* TD는 MC와 다르게 next time step까지만 기다린다.
  ...
  * <img src="https://i.loli.net/2020/10/23/Z2ctdA6YiTEWuoH.png" alt="image-20201023163536850" style="zoom:67%;" />

* 식에서 확인할 수 있듯이, MC의 update는 $G_t$이고, TD의 update는 $R_{t+1}+\gamma V(S_{t+1})$ 이다.
...
* TD error란, $S_t$의 기존 추정값과 더 나은 추정값 $R_{t+1}+\gamma V\left(S_{t+1}\right)$ 의 차이를 말한다.
...
* TD error는 그 시간대에서만 유효하고, 이후에는 더 이상 유효하지 않다.
  ...
  * 즉, $V(S_t)$의 에러 $\delta_t$는 $t+1$에서만 유효하다.
...
* $$
  ...
  * <img src="https://i.loli.net/2020/10/23/u2LqvWVTtze8Ujr.png" alt="image-20201023172646120" style="zoom:67%;" />
  * 여기서 $S_{t+1}$가 terminal이면,  $Q\left(S_{t+1}, A_{t+1}\right)$ 은 0으로 정한다.

* 매 학습마다 $\left(S_{t}, A_{t}, R_{t+1}, S_{t+1}, A_{t+1}\right)$ 를 사용하기 때문에 SARSA라 불린다.
