...
$$
\min _{\theta} C\left[\sum_{i=1}^{m} y^{(i)} \text{cost}_{1}\left(\theta^{T} x^{(i)}\right)+\left(1-y^{(i)}\right) \text{cost}_{0}\left(\theta^{T} x^{(i)}\right)\right]+\frac{1}{2} \sum_{j=1}^{n} \theta_{j}^{2}
$$
...
$$
\min _{\theta} C * 0+\frac{1}{2} \sum_{j=1}^{n} \theta_{j}^{2}=\min _{\theta} \sum_{j=1}^{n} \theta_{j}^{2}
$$
