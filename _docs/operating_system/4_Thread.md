...
1. 사용자 수준 쓰레드 - 다대일 매핑
   ...
   * 커널에 독립적으로 스케쥴링 할 수 있으므로 (스레드에 관련된 모든 행위는 커널이 관여하지 않는다), 이식성이 높고 유연하다.
   * 다만, 쓰레드가 아닌 프로세스 단위로 프로세서를 할당하여 다중 처리 환경을 갖춰도 쓰레드 단위로 다중 처리를 하지 못한다.
2. 커널 수준 쓰레드 - 일대일 매핑
   * 커널 수준 쓰레드는 사용자 수준 쓰레드와 커널 수준 쓰레드가 일대일로 매핑된다. 따라서 사용자 수준 쓰레드를 생성하면 이에 대응하는 커널 쓰레드를 자동으로 생성한다.
   * 커널 수준 쓰레드는 커널이 직접 스케줄링하고 실행하기에 사용자 수준 쓰레드의 커널 지원이 부족한 문제를 해결할 수 있다.
   * 하지만, 커널이 전체 프로세스와 쓰레드 정보를 유지하여 오버헤드가 커진다.
3. 혼합형 쓰레드 - 다대다 매핑
   * 혼합형 쓰레드는 사용자 수준 쓰레드와 커널 수준 쓰레드를 혼합한 구조이다.

![img](https://i.loli.net/2020/12/02/4RYPOtl8qk9mDwn.png)