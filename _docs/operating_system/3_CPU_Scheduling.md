...
2. 라운드 로빈(Round-Robin, RR) 스케줄링
   ...
   * 할당되는 시간이 길어질수록, 비선점 방식인 FCFS와 비슷해짐 
3. 다단계 큐(Multi-Level Queue) 스케줄링
   * 어떤 프소세스냐에 따라서 여러 종류의 그룹으로 나누고, 여러개의 큐마다 다양한 알고리즘을 적용하는 스케줄링 기법
   * ![img](https://i.loli.net/2020/12/01/oseYUBDMvLmaNdj.png)
4. 다단계 피드백 큐(Multi-Level Feedback Queue) 스케줄링
   * 다단계 큐의 단점을 보안한 스케줄링
   * 큐마다 timeout을 설정하여 timeout 초과시, 우선순위가 낮은 다음단계 큐로 이동함
...
2. HRN(Highest response ratio next) 스케줄링
...
3. FCFS (FIrst Come First Served Scheuling, 또는 FIFO)스케줄링
