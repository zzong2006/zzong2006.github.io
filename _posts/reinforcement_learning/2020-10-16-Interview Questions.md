...
1. ### 감독학습, 무감독 학습과 비교하여 강화학습이 다른 점은 무엇인가? 
...
2. ### 마코프결정문제(MDP)를 정의하시오.
   ...
   * <State $S$, Action $A$ , transition probability $P$, Reward $R$, discount factor $\gamma$> 라는 tuple로 정의됨
     * Transition probability $P$:  현재 상태 $s$에서 $a$라는 행동을 취할 때 $s′$로 이동할 확률($p(s′|s, a)$)을 모아놓은 집합을 의미 
...
4. ### 벨만 방적식 (Bellman Equation)
    ...
    * policy를 포함한 value function과 action value function도 벨만 방정식으로 표현해보자.
    ...
    * 좀 더 시각적으로 벨만 방정식의 관계에 대해 이해해보자.    
    ...
    * #### Optimal (action/state-)value function (about Bellman Optimality Equation)
      ...
      * MDP에서 action의 확률은 deterministic하므로(변하지 않음), 최적의 정책 $\pi_*(a|s)$은 아래와 같이 표현된다.
...
5. ### Dynamic Programming
    ...
    * DP는 수학적으로 잘 정돈된 방법이지만, 환경에 대한 완벽하고 정확한 모델을 요구한다.    
      즉, Planning이다.  

    * #### Planning & Learning

      * Planning: 환경의 모델을 이미 안 상태에서 agent가 계산을 수행하는 것 (search)
    ...
    * DP는 두 step: 1) prediction과 2) control을 반복한다.

      1. Prediction : 현재 optimal하지 않은 policy에 대해서 value function을 구한다.
      2. Control: 현재의 value function을 토대로 더 나은 policy를 구한다.
...
8. ## 강화학습의 다양한 전략들을 기술하고 서로간의 차이를 설명하시오. 
   ...
   1. ### 모델기반 vs. 모델프리 RL의 차이를 설명하시오. 
      
      * 
   2. ### 오프라인 vs. 온라인 RL의 차이를 설명하시오. 
      
      * 
   3. ### On-policy vs. Off-policy RL의 차이를 설명하시오. 
      
      * 

9. 

10. ### TD학습, SARSA, Q학습 알고리즘을 설명하고 차이점을 기술하시오

11. ### Policy Gradient에 대해서 설명하라
